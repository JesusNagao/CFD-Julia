...
function Poisson(P::PressureField2D, exportocsv = false)
    ...
    for i in range(1,stop=nt)
        ...
        P.p[2:end-1,2:end-1] = ((dx^2 *(pcopy[2:end-1,3:end] + pcopy[2:end-1,1:end-2]) + 
        dy^2 * (pcopy[3:end, 2:end-1] + pcopy[1:end-2, 2:end-1]) - 
...
function PlotP(P::PressureField2D, exportcsv = false)
    ...
    a = plot(X,Y,P.p[:,:],st = :surface, xlims = (0,2), ylims = (0,2), camera=(0,30), c = cgrad(:ice))
    savefig(a, "Poiss.png")
